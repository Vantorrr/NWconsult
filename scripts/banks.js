...
(function(){
  ...
  const defaults = [
    { country:'Швейцария', bank:'Private/Corporate', channels:'SWIFT', remote:true, minBalance:'50k+', notes:'' },
    { country:'ОАЭ', bank:'Local/International', channels:'SWIFT', remote:false, minBalance:'10k+', notes:'' },
    { country:'EMI (Европа)', bank:'IBAN/SEPA', channels:'SEPA, SWIFT', remote:true, minBalance:'0–5k', notes:'' },
    { country:'Гонконг', bank:'Corporate', channels:'SWIFT', remote:false, minBalance:'10k+', notes:'' },
    { country:'Сингапур', bank:'Corporate', channels:'SWIFT', remote:false, minBalance:'20k+', notes:'' }
  ];
    ...
    data.forEach((r, i)=>{
      ...
      tr.innerHTML = `
        ...
        <td>
          ...
          <button class="btn" data-remove="${i}">Удалить</button>
        </td>`;
  ...
  function openDialog(r, onSave){
    const country = prompt('Страна', r?.country||''); if(!country) return;
    const bank = prompt('Тип / Банк', r?.bank||'');
    const channels = prompt('Платежные каналы', r?.channels||'SWIFT');
    const remote = confirm('Возможна удаленная подача? Ок = Да, Отмена = Нет');
    const minBalance = prompt('Мин. остаток', r?.minBalance||'');
